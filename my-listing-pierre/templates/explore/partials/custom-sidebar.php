...
<div class="finder-tabs col-md-12 <?php echo count( $explore->types ) > 1 ? 'with-listing-types' : 'without-listing-types' ?>">
	...
	<?php foreach ( $explore->types as $type ):
		...
		<ul class="hide sidebar-tabs <?php echo 'tab-count-'.count($tabs) ?>" role="tablist" v-show="activeType.slug === '<?php echo esc_attr( $type->get_slug() ) ?>'">
			...
			<div id="search-form" class="listing-type-filters search-tab tab-pane fade" :class="activeType.tab == 'search-form' ? 'in active' : ''">
				<div class="search-filters type-<?php echo esc_attr( $type->get_slug() ) ?> type-id-<?php echo absint( $type->get_id() ) ?>">
					<div class="light-forms filter-wrapper">
						<?php 
						$filters = $type->get_advanced_filters();
						foreach ( $filters as $filter ) :
							if ( ! in_array( $filter->get_type(), [ 'group-start', 'group-end', 'location', 'wp-search', 'proximity', 'order' ] ) ) {
								continue;
							}
						?>
						...
						?>
							<div class="panel-dropdown wide <?php echo $active; ?>">
			...
			<transition-group name="vfade-down">
				<div v-if="currentTax.activeTerm" class="active-term" :key="currentTax.activeTerm.term_id">
					...
					<div class="active-taxonomy-container" :class="currentTax.activeTerm.background ? 'with-bg' : 'no-bg'">
						...
						<p class="category-description" v-html="currentTermDescription"></p>
