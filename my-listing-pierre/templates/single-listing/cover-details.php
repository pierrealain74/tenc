        ...
        switch ( $detail['format'] ) {
            ...
            case 'date':
            	if (str_contains($field_value, ' - ')) {
            		$dateArray = explode(' - ', $field_value, 2);
            		$date1 = date_i18n( get_option( 'date_format' ), strtotime( $dateArray[0] ) );
            		$date2 = date_i18n( get_option( 'date_format' ), strtotime( $dateArray[1] ) );
            ...
            case 'datetime':
            	if (str_contains($field_value, ' - ')) {
            		$dateArray = explode(' - ', $field_value, 2);
            		$date1 = date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $dateArray[0] ) );
            		$date2 = date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $dateArray[1] ) );
            		$field_value = $date1 . ' - ' . $date2;
            	} else {
            ...
            case 'time':
	            if (str_contains($field_value, ' - ')) {
	            	$dateArray = explode(' - ', $field_value, 2);
	            	$date1 = date_i18n( get_option( 'time_format' ), strtotime( $dateArray[0] ) );
	            	$date2 = date_i18n( get_option( 'time_format' ), strtotime( $dateArray[1] ) );
	            	$field_value = $date1 . ' - ' . $date2;
	            } else {
