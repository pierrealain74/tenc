...
abstract class Base_Profile_Field implements \JsonSerializable {
	...
	protected function get_icon_field() {?>
		<div class="form-group" >
			<label>Icon</label>
	...
	protected function get_options_field() { ?>
		<div class="form-group options-field field-options-input">
			<label>Options</label>

			<div class="options-list" v-show="!state.editingOptions">
			...
			<div v-show="!state.editingOptions && !Object.keys(field.options).length">
				<small><em>No options added yet.</em></small>
			</div>

			<textarea
				id="custom_field_options"
				v-show="state.editingOptions"
			...
			<small v-show="state.editingOptions"><em>Put each option in a new line. You can specify both a value and label like this: <code>red : Red</code></em></small>
			<br><br v-show="state.editingOptions || Object.keys(field.options).length">
			<button @click.prevent="state.editingOptions = !state.editingOptions;" class="btn btn-primary">{{ state.editingOptions ? 'Save Options' : 'Add/Edit Options' }}</button>
		</div>
	<?php }
}
