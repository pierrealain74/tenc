<reviews-profile inline-template v-if="currentSubTab === 'reviews-profile'">
	<div class="tab-content full-width">
		<div class="form-section">
			<h3>Listing Reviews Profiles</h3>
		</div>
		...
		<div class="editor-column col-1-2 rows row-padding">
			...
			<div class="form-section mb40">
				...
				<label class="form-switch">
					<input type="checkbox" v-model="$root.settings.reviewsProfile.ratings.enabled">
					<span class="switch-slider"></span>
				</label>

				<div class="mt10" v-show="!$root.settings.reviewsProfile.ratings.enabled">
					<p>Allow users to submit multiple comments?</p>
					<label class="form-switch">
						<input type="checkbox" v-model="$root.settings.reviewsProfile.multiple">
						<span class="switch-slider"></span>
					</label>
				</div>
			</div>

			<div class="form-section" :class="!$root.settings.reviewsProfile.ratings.enabled ? 'ml-overlay-disabled' : ''">
				...
				<div class="form-group mb10">
					<label>
						<input type="radio" v-model="$root.settings.reviewsProfile.ratings.mode" value="5" class="form-radio">
						<span>Full stars</span>
					</label>
				</div>
				<div class="form-group">
					<label>
						<input type="radio" v-model="$root.settings.reviewsProfile.ratings.mode" value="10" class="form-radio">
						<span>Half stars</span>
					</label>
				</div>
			</div>
		</div><!--
		--><div class="editor-column col-1-2 rows row-padding" :class="!$root.settings.reviewsProfile.ratings.enabled ? 'ml-overlay-disabled' : ''">
			<h4>Rating Categories</h4>

			<draggable v-model="$root.settings.reviewsProfile.ratings.categories" :options="{group: 'settings-reviews-categories', handle: '.row-head'}">
				<div v-for="category in $root.settings.reviewsProfile.ratings.categories" class="row-item" :class="isActive( category ) ? 'open' : ''">
					<div @click="activeCategory = ( isActive( category ) ? null : category )" class="row-head">
						...
						<div class="row-head-actions">
							<span
								...
								v-show="$root.settings.reviewsProfile.ratings.categories.length > 1 && category.id !== 'rating'"
